...
namespace Tests {
  ...
  namespace po = boost::program_options;
    ...
    public:
      BaseFunctionalTests(const UpscaleCoin::Currency& currency, System::Dispatcher& d, const BaseFunctionalTestsConfig& config) :
          m_dispatcher(d),
          m_currency(currency),
    ...
    protected:
      ...
      bool makeWallet(std::unique_ptr<UpscaleCoin::IWalletLegacy> & wallet, std::unique_ptr<UpscaleCoin::INode>& node, const std::string& password = "pass");
      bool mineBlocks(TestNode& node, const UpscaleCoin::AccountPublicAddress& address, size_t blockCount);
      bool mineBlock(std::unique_ptr<UpscaleCoin::IWalletLegacy>& wallet);
      bool mineBlock();
      bool startMining(size_t threads);
      bool stopMining();

      bool getNodeTransactionPool(size_t nodeIndex, UpscaleCoin::INode& node, std::vector<std::unique_ptr<UpscaleCoin::ITransactionReader>>& txPool);

      bool waitDaemonsReady();
      bool waitDaemonReady(size_t nodeIndex);
      bool waitForPeerCount(UpscaleCoin::INode& node, size_t expectedPeerCount);
      bool waitForPoolSize(size_t nodeIndex, UpscaleCoin::INode& node, size_t expectedPoolSize,
        std::vector<std::unique_ptr<UpscaleCoin::ITransactionReader>>& txPool);

      bool prepareAndSubmitBlock(TestNode& node, UpscaleCoin::Block&& blockTemplate);
    ...
    private:
      ...
      std::unique_ptr<UpscaleCoin::INode> mainNode;
      std::unique_ptr<UpscaleCoin::IWalletLegacy> workingWallet;
