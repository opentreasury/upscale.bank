...
#ifdef _WIN32
const std::string daemonExec = std::string(UpscaleCoin::UpscaleCoin_NAME) + "d.exe";
#else
const std::string daemonExec = std::string(UpscaleCoin::UpscaleCoin_NAME) + "d";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.testnet = testnet;
  cfg.logFile = std::string("test_") + UpscaleCoin::UpscaleCoin_NAME + "d" + std::to_string(index) + ".log";
...
TestNetwork::TestNetwork(System::Dispatcher& dispatcher, const UpscaleCoin::Currency& currency) :
