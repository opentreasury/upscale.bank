...
bool RPCTestNode::getBlockTemplate(const std::string& minerAddress, UpscaleCoin::Block& blockTemplate, uint64_t& difficulty) {
...
bool RPCTestNode::makeINode(std::unique_ptr<UpscaleCoin::INode>& node) {
  std::unique_ptr<UpscaleCoin::INode> newNode(new UpscaleCoin::NodeRpcProxy("127.0.0.1", m_rpcPort));
  NodeCallback cb;
...
uint64_t RPCTestNode::getLocalHeight() {
  try {
    UpscaleCoin::COMMAND_RPC_GET_INFO::request req;
    UpscaleCoin::COMMAND_RPC_GET_INFO::response rsp;
