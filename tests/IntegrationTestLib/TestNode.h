...
public:
  ...
  virtual bool getBlockTemplate(const std::string& minerAddress, UpscaleCoin::Block& blockTemplate, uint64_t& difficulty) = 0;
  virtual bool submitBlock(const std::string& block) = 0;
  virtual bool getTailBlockId(Crypto::Hash& tailBlockId) = 0;
  virtual bool makeINode(std::unique_ptr<UpscaleCoin::INode>& node) = 0;
  virtual uint64_t getLocalHeight() = 0;

  std::unique_ptr<UpscaleCoin::INode> makeINode() {
