...
InProcTestNode::InProcTestNode(const TestNodeConfiguration& cfg, const UpscaleCoin::Currency& currency) :
...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    ...
    core.reset(new UpscaleCoin::core(m_currency, NULL, log));
    protocol.reset(new UpscaleCoin::UpscaleCoinProtocolHandler(m_currency, dispatcher, *core, NULL, log));
    p2pNode.reset(new UpscaleCoin::NodeServer(dispatcher, *protocol, log));
    protocol->set_p2p_endpoint(p2pNode.get());
    core->set_UpscaleCoin_protocol(protocol.get());

    UpscaleCoin::NetNodeConfig p2pConfig;
    ...
    UpscaleCoin::MinerConfig emptyMiner;
  ...
  core->deinit();
  p2pNode->deinit();
  core->set_UpscaleCoin_protocol(NULL);
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, UpscaleCoin::Block &blockTemplate, uint64_t &difficulty) {
  AccountPublicAddress addr;
...
bool InProcTestNode::makeINode(std::unique_ptr<UpscaleCoin::INode> &node) {

  std::unique_ptr<InProcessNode> inprocNode(new UpscaleCoin::InProcessNode(*core, *protocol));
