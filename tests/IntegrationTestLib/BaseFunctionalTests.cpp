...
#ifdef _WIN32
const std::string DAEMON_FILENAME = std::string(UpscaleCoin_NAME) + "d.exe";
#else
const std::string DAEMON_FILENAME = std::string(UpscaleCoin_NAME) + "d";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_" << UpscaleCoin_NAME << "d_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"" + std::string(UpscaleCoin_NAME) + "d" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), (std::string(UpscaleCoin_NAME) + "d").c_str(), "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
...
namespace {
  class WaitForCoinBaseObserver : public UpscaleCoin::IWalletLegacyObserver {
    Semaphore& m_gotReward;
    UpscaleCoin::IWalletLegacy& m_wallet;
  public:
    WaitForCoinBaseObserver(Semaphore& gotReward, UpscaleCoin::IWalletLegacy& wallet) : m_gotReward(gotReward), m_wallet(wallet) { }
    virtual void externalTransactionCreated(UpscaleCoin::TransactionId transactionId) override {
      UpscaleCoin::WalletLegacyTransaction trInfo;
...
bool BaseFunctionalTests::mineBlocks(TestNode& node, const UpscaleCoin::AccountPublicAddress& address, size_t blockCount) {
...
bool BaseFunctionalTests::prepareAndSubmitBlock(TestNode& node, UpscaleCoin::Block&& blockTemplate) {
  blockTemplate.timestamp = m_nextTimestamp;
  m_nextTimestamp += 2 * m_currency.difficultyTarget();

  BinaryArray blockBlob = UpscaleCoin::toBinaryArray(blockTemplate);
  return node.submitBlock(::Common::toHex(blockBlob.data(), blockBlob.size()));
}

bool BaseFunctionalTests::mineBlock(std::unique_ptr<UpscaleCoin::IWalletLegacy> &wallet) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<UpscaleCoin::IWalletLegacy> & wallet, std::unique_ptr<UpscaleCoin::INode>& node, const std::string& password) {
  if (!node) return false;
  wallet = std::unique_ptr<UpscaleCoin::IWalletLegacy>(new UpscaleCoin::WalletLegacy(m_currency, *node));
...
namespace {
  struct PeerCountWaiter : UpscaleCoin::INodeObserver {
...
bool BaseFunctionalTests::waitForPeerCount(UpscaleCoin::INode& node, size_t expectedPeerCount) {
...
bool BaseFunctionalTests::waitForPoolSize(size_t nodeIndex, UpscaleCoin::INode& node, size_t expectedPoolSize,
  std::vector<std::unique_ptr<UpscaleCoin::ITransactionReader>>& txPool) {
...
bool BaseFunctionalTests::getNodeTransactionPool(size_t nodeIndex, UpscaleCoin::INode& node,
  std::vector<std::unique_ptr<UpscaleCoin::ITransactionReader>>& txPool) {
