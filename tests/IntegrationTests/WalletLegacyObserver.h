...
public:
  ...
  UpscaleCoin::TransactionId waitExternalTransaction() {
    std::unique_lock<std::mutex> lk(m_mutex);
    ...
    UpscaleCoin::TransactionId txId = m_externalTransactions.front();
  ...
  std::error_code waitSendResult(UpscaleCoin::TransactionId txid) {
    std::unique_lock<std::mutex> lk(m_mutex);

    std::unordered_map<UpscaleCoin::TransactionId, std::error_code>::iterator it;
...
private:
  ...
  std::unordered_map<UpscaleCoin::TransactionId, std::error_code> m_sendResults;
  std::deque<UpscaleCoin::TransactionId> m_externalTransactions;
};
