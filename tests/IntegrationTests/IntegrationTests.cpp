...
public:
  ...
  IntegrationTest() : 
    currency(UpscaleCoin::CurrencyBuilder(log).testnet(true).currency()),
  ...
  void makeWallets() {
    for (auto& n: inodes) {
      std::unique_ptr<UpscaleCoin::IWalletLegacy> wallet(new UpscaleCoin::WalletLegacy(m_currency, *n));
...
TEST_F(IntegrationTest, BlockPropagationSpeed) {
  ...
  {
    std::unique_ptr<UpscaleCoin::INode>& localNode = inodes.front();
    std::unique_ptr<UpscaleCoin::INode>& remoteNode = inodes.back();
