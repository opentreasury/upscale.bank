...
public:
  ...
  SimpleTest(const UpscaleCoin::Currency& currency, System::Dispatcher& system, const Tests::Common::BaseFunctionalTestsConfig& config) :
    BaseFunctionalTests(currency, system, config) {}

  class WaitForActualGrowObserver : public UpscaleCoin::IWalletLegacyObserver {
  ...
  class WaitForActualDwindleObserver : public UpscaleCoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForPendingGrowObserver : public UpscaleCoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForConfirmationObserver : public UpscaleCoin::IWalletLegacyObserver {
  ...
  class WaitForSendCompletedObserver : public UpscaleCoin::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_Sent;
    std::error_code& m_error;
    UpscaleCoin::TransactionId& m_transactionId;

  public:
    WaitForSendCompletedObserver(Tests::Common::Semaphore& Sent, UpscaleCoin::TransactionId& transactionId, std::error_code& error) : m_Sent(Sent), m_transactionId(transactionId), m_error(error) { }
  ...
  class WaitForExternalTransactionObserver : public UpscaleCoin::IWalletLegacyObserver {
  public:
    WaitForExternalTransactionObserver() { }
    std::promise<UpscaleCoin::TransactionId> promise;
  ...
  class WaitForTransactionUpdated : public UpscaleCoin::IWalletLegacyObserver {
  public:
    ...
    UpscaleCoin::TransactionId expectindTxId;
  };
  ...
  bool perform1() {
    ...
    std::unique_ptr<UpscaleCoin::INode> node1;
    std::unique_ptr<UpscaleCoin::INode> node2;

    nodeDaemons.front()->makeINode(node1);
    nodeDaemons.front()->makeINode(node2);

    std::unique_ptr<UpscaleCoin::IWalletLegacy> wallet1;
    std::unique_ptr<UpscaleCoin::IWalletLegacy> wallet2;
  ...
  class WaitForBlockchainHeightChangeObserver : public UpscaleCoin::INodeObserver {
  ...
  class CallbackHeightChangeObserver : public UpscaleCoin::INodeObserver {
  ...
  bool perform2(size_t blocksCount = 10)
  {
    ...
    mineBlock();
    LOG_TRACE("STEP 2 PASSED");
    std::unique_ptr<UpscaleCoin::INode> localNode;
    std::unique_ptr<UpscaleCoin::INode> remoteNode;

    nodeDaemons.front()->makeINode(localNode);
    nodeDaemons.back()->makeINode(remoteNode);
    
    std::unique_ptr<UpscaleCoin::IWalletLegacy> wallet;
  ...
  bool perform4() {
    ...
    std::unique_ptr<UpscaleCoin::INode> hopNode;
    std::unique_ptr<UpscaleCoin::INode> localNode;
    std::unique_ptr<UpscaleCoin::INode> remoteNode;
  ...
  bool perform5() {
    ...
    std::unique_ptr<UpscaleCoin::INode> node1;
    std::unique_ptr<UpscaleCoin::INode> inprocNode;
...
void testMultiVersion(const UpscaleCoin::Currency& currency, System::Dispatcher& d, const Tests::Common::BaseFunctionalTestsConfig& config);
...
public:
  ...
  SimpleTestCase() : 
    currency(UpscaleCoin::CurrencyBuilder(logger).testnet(true).currency()),
...
TEST_F(SimpleTestCase, TESTPOOLDELETION) {
  currency = UpscaleCoin::CurrencyBuilder(logger).testnet(true).mempoolTxLiveTime(60).currency();
