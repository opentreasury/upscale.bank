...
//#include "UpscaleCoin_boost_serialization.h"

using namespace std;
using namespace UpscaleCoin;

struct output_index {
    const UpscaleCoin::TransactionOutputTarget out;
    uint64_t amount;
    ...
    uint32_t idx;
    bool spent;
    const UpscaleCoin::Block *p_blk;
    const UpscaleCoin::Transaction *p_tx;

    output_index(const UpscaleCoin::TransactionOutputTarget &_out, uint64_t _a, size_t _h, size_t tno, size_t ono, const UpscaleCoin::Block *_pb, const UpscaleCoin::Transaction *_pt)
...
bool init_output_indices(map_output_idx_t& outs, std::map<uint64_t, std::vector<size_t> >& outs_mine, const std::vector<UpscaleCoin::Block>& blockchain, const map_hash2tx_t& mtx, const UpscaleCoin::AccountBase& from) {
...
bool init_spent_output_indices(map_output_idx_t& outs, map_output_t& outs_mine, const std::vector<UpscaleCoin::Block>& blockchain, const map_hash2tx_t& mtx, const UpscaleCoin::AccountBase& from) {
...
bool fill_tx_sources(std::vector<TransactionSourceEntry>& sources, const std::vector<test_event_entry>& events,
                     const Block& blk_head, const UpscaleCoin::AccountBase& from, uint64_t amount, size_t nmix)
{
    map_output_idx_t outs;
    map_output_t outs_mine;

    std::vector<UpscaleCoin::Block> blockchain;
    map_hash2tx_t mtx;
    ...
    BOOST_REVERSE_FOREACH(const map_output_t::value_type o, outs_mine)
    {
        for (size_t i = 0; i < o.second.size() && !sources_found; ++i)
        {
            ...
            UpscaleCoin::TransactionSourceEntry ts;
            ts.amount = oi.amount;
...
bool fill_tx_destination(TransactionDestinationEntry &de, const UpscaleCoin::AccountBase &to, uint64_t amount) {
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, UpscaleCoin::Transaction& tx, const Block& blk_head,
                         const UpscaleCoin::AccountBase& from, const UpscaleCoin::AccountBase& to, uint64_t amount,
...
const UpscaleCoin::Currency& test_chain_unit_base::currency() const
{
  return m_currency;
}
...
bool test_chain_unit_base::verify(const std::string& cb_name, UpscaleCoin::core& c, size_t ev_index, const std::vector<test_event_entry> &events)
{
