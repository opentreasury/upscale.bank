...
bool GetRandomOutputs::request(UpscaleCoin::core& c, uint64_t amount, size_t mixin, UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp) {
  UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request req;

  req.amounts.push_back(amount);
  req.outs_count = mixin;

  resp = boost::value_initialized<UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response>();
...
bool GetRandomOutputs::checkHalfUnlocked(UpscaleCoin::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
...
bool GetRandomOutputs::checkFullyUnlocked(UpscaleCoin::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
