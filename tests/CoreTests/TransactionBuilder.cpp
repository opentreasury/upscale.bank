...
TransactionBuilder::TransactionBuilder(const UpscaleCoin::Currency& currency, uint64_t unlockTime)
  : m_currency(currency), m_version(UpscaleCoin::CURRENT_TRANSACTION_VERSION), m_unlockTime(unlockTime), m_txKey(generateKeyPair()) {}
...
TransactionBuilder& TransactionBuilder::setTxKeys(const UpscaleCoin::KeyPair& txKeys) {
  m_txKey = txKeys;
  return *this;
}

TransactionBuilder& TransactionBuilder::setInput(const std::vector<UpscaleCoin::TransactionSourceEntry>& sources, const UpscaleCoin::AccountKeys& senderKeys) {
  m_sources = sources;
...
TransactionBuilder& TransactionBuilder::setOutput(const std::vector<UpscaleCoin::TransactionDestinationEntry>& destinations) {
  m_destinations = destinations;
  return *this;
}

TransactionBuilder& TransactionBuilder::addOutput(const UpscaleCoin::TransactionDestinationEntry& dest) {
...
Transaction TransactionBuilder::build() const {
  ...
  std::vector<UpscaleCoin::KeyPair> contexts;
...
void TransactionBuilder::fillInputs(Transaction& tx, std::vector<UpscaleCoin::KeyPair>& contexts) const {
...
void TransactionBuilder::signSources(const Crypto::Hash& prefixHash, const std::vector<UpscaleCoin::KeyPair>& contexts, Transaction& tx) const {
