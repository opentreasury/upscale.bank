...
#include "UpscaleCoinCore/Miner.h"
...
uint64_t test_generator::getAlreadyGeneratedCoins(const UpscaleCoin::Block& blk) const {
  Crypto::Hash blkHash;
  get_block_hash(blk, blkHash);
  return getAlreadyGeneratedCoins(blkHash);
}

void test_generator::addBlock(const UpscaleCoin::Block& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(UpscaleCoin::Block& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
...
bool test_generator::constructBlock(UpscaleCoin::Block& blk, const UpscaleCoin::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<UpscaleCoin::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(UpscaleCoin::Block& blk, const UpscaleCoin::Block& blkPrev,
                                    const UpscaleCoin::AccountBase& minerAcc,
...
bool test_generator::constructBlockManuallyTx(UpscaleCoin::Block& blk, const UpscaleCoin::Block& prevBlock,
                                              const UpscaleCoin::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(UpscaleCoin::Block& blk, const UpscaleCoin::Block& blkPrev,
                                           const UpscaleCoin::AccountBase& minerAccount,
...
void fillNonce(UpscaleCoin::Block& blk, const difficulty_type& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxBySize(const UpscaleCoin::Currency& currency, UpscaleCoin::Transaction& baseTransaction, uint32_t height,
  ...
  if (!currency.constructMinerTx(height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, UpscaleCoin::BinaryArray(), 1)) {
    return false;
  }
