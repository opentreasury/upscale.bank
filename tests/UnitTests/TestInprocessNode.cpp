...
namespace {
UpscaleCoin::Transaction createTx(UpscaleCoin::ITransactionReader& tx) {
  UpscaleCoin::Transaction outTx;
...
protected:
  ...
  ICoreStub coreStub;
  IUpscaleCoinProtocolQueryStub protocolQueryStub;
  UpscaleCoin::InProcessNode node;
...
TEST_F(InProcessNodeTests, initOk) {
  UpscaleCoin::InProcessNode newNode(coreStub, protocolQueryStub);
  CallbackStatus status;
...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsSuccess) {
  ...
  UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response expectedResp;
  UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount out;
  out.amount = 10;
  out.outs.push_back({ 11, ignoredPublicKey });
  expectedResp.outs.push_back(out);
  coreStub.set_random_outs(expectedResp, true);

  std::vector<UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount> outs;
...
TEST_F(InProcessNodeTests, getNewBlocksUninitialized) {
  ...
  std::vector<UpscaleCoin::block_complete_entry> newBlocks;
...
TEST_F(InProcessNodeTests, relayTransactionUninitialized) {
  ...
  CallbackStatus status;
  newNode.relayTransaction(UpscaleCoin::Transaction(), [&] (std::error_code ec) { status.setStatus(ec); });
  ASSERT_TRUE(status.wait());
  ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHeightEmpty) {
  ...
  std::vector<std::vector<UpscaleCoin::BlockDetails>> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHeightMany) {
  ...
  std::vector<uint32_t> blockHeights;
  std::vector<std::vector<UpscaleCoin::BlockDetails>> actualBlocks;

  std::vector<UpscaleCoin::Block> expectedBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    ...
    blockHeights.push_back(std::move(boost::get<UpscaleCoin::BaseInput>(iter->baseTransaction.inputs.front()).blockIndex));
  ...
  for (const boost::tuple<boost::tuple<size_t, UpscaleCoin::Block>, std::vector<UpscaleCoin::BlockDetails>>& sameHeight : range) {
    EXPECT_EQ(sameHeight.get<1>().size(), 1);
    for (const UpscaleCoin::BlockDetails& block : sameHeight.get<1>()) {
      EXPECT_EQ(block.height, sameHeight.get<0>().get<0>());
      Crypto::Hash expectedCryptoHash = UpscaleCoin::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getBlocksByHeightFail) {
  ...
  for (const UpscaleCoin::Block& block : generator.getBlockchain()) {
...
TEST_F(InProcessNodeTests, getBlocksByHashEmpty) {
  ...
  std::vector<UpscaleCoin::BlockDetails> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHashMany) {
  ...
  std::vector<Crypto::Hash> blockHashes;
  std::vector<UpscaleCoin::BlockDetails> actualBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    expectedBlocks.push_back(*iter);
    blockHashes.push_back(UpscaleCoin::get_block_hash(*iter));
    coreStub.addBlock(*iter);
  }
  ...
  auto range = boost::combine(range1, actualBlocks);
  for (const boost::tuple<boost::tuple<Crypto::Hash, UpscaleCoin::Block>, UpscaleCoin::BlockDetails>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = UpscaleCoin::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getTxEmpty) {
  ...
  std::vector<UpscaleCoin::TransactionDetails> transactions;
...
TEST_F(InProcessNodeTests, getTxMany) {
  ...
  std::vector<Crypto::Hash> transactionHashes;
  std::vector<UpscaleCoin::TransactionDetails> actualTransactions;

  std::vector<std::tuple<UpscaleCoin::Transaction, Crypto::Hash, uint64_t>> expectedTransactions;

  coreStub.set_blockchain_top(0, boost::value_initialized<Crypto::Hash>());

  size_t prevBlockchainSize = generator.getBlockchain().size();
  for (size_t i = 0; i < BLOCKCHAIN_TX_NUMBER; ++i) {
    auto txptr = UpscaleCoin::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.push_back(UpscaleCoin::getObjectHash(tx));
    ...
    expectedTransactions.push_back(std::make_tuple(tx, UpscaleCoin::get_block_hash(generator.getBlockchain().back()), boost::get<UpscaleCoin::BaseInput>(generator.getBlockchain().back().baseTransaction.inputs.front()).blockIndex));
  }
  ...
  for (const boost::tuple<boost::tuple<Crypto::Hash, UpscaleCoin::TransactionDetails>, std::tuple<UpscaleCoin::Transaction, Crypto::Hash, uint64_t>>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = UpscaleCoin::getObjectHash(std::get<0>(sameHeight.get<1>()));
    EXPECT_EQ(expectedCryptoHash, sameHeight.get<0>().get<0>());
    Hash expectedHash = reinterpret_cast<const Hash&>(expectedCryptoHash);
...
TEST_F(InProcessNodeTests, getLastLocalBlockTimestamp) {
  ...
  UpscaleCoin::InProcessNode newNode(core, protocolQueryStub);
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC) {
  ...
  for (size_t i = 0; i < POOL_TX_NUMBER; ++i) {
    auto txptr = UpscaleCoin::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.insert(UpscaleCoin::getObjectHash(tx));
    UpscaleCoin::tx_verification_context tvc = boost::value_initialized<tx_verification_context>();
    bool keptByBlock = false;
    coreStub.handleIncomingTransaction(tx, UpscaleCoin::getObjectHash(tx), UpscaleCoin::getObjectBinarySize(tx), tvc, keptByBlock);
  ...
  std::vector<Crypto::Hash> knownPoolTxIds;
  Crypto::Hash knownBlockId = UpscaleCoin::getObjectHash(generator.getBlockchain().back());
  bool isBcActual = false;
  std::vector<std::unique_ptr<ITransactionReader>> newTxs;
  std::vector<Crypto::Hash> deletedTxIds;
