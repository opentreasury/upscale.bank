...
class TrivialWalletObserver : public UpscaleCoin::IWalletLegacyObserver
{
public:
...
struct SaveOnInitWalletObserver: public UpscaleCoin::IWalletLegacyObserver {
  SaveOnInitWalletObserver(UpscaleCoin::WalletLegacy* wallet) : wallet(wallet) {
  };
  ...
  UpscaleCoin::WalletLegacy* wallet;
  std::stringstream stream;
};

static const uint64_t TEST_BLOCK_REWARD = 70368744177663;

UpscaleCoin::TransactionId TransferMoney(UpscaleCoin::WalletLegacy& from, UpscaleCoin::WalletLegacy& to, int64_t amount, uint64_t fee, uint64_t mixIn = 0, const std::string& extra = "") {
  UpscaleCoin::WalletLegacyTransfer transfer;
...
{
public:
  WalletLegacyApi() : m_currency(UpscaleCoin::CurrencyBuilder(m_logger).currency()), generator(m_currency) {
  }
...
protected:
  ...
  void GetOneBlockReward(UpscaleCoin::WalletLegacy& wallet);
  void GetOneBlockReward(UpscaleCoin::WalletLegacy& wallet, const UpscaleCoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
  void GetOneBlockRewardAndUnlock(UpscaleCoin::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                  const UpscaleCoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
...
void WalletLegacyApi::prepareAliceWallet() {
  ...
  alice.reset(new UpscaleCoin::WalletLegacy(m_currency, *newNode));
  aliceNode = newNode;
...
void WalletLegacyApi::prepareBobWallet() {
  ...
  bob.reset(new UpscaleCoin::WalletLegacy(m_currency, *bobNode));
...
void WalletLegacyApi::prepareCarolWallet() {
  ...
  carol.reset(new UpscaleCoin::WalletLegacy(m_currency, *carolNode));
  carol->addObserver(carolWalletObserver.get());
}

void WalletLegacyApi::GetOneBlockReward(UpscaleCoin::WalletLegacy& wallet) {
  GetOneBlockReward(wallet, m_currency, generator);
}

void WalletLegacyApi::GetOneBlockReward(UpscaleCoin::WalletLegacy& wallet, const UpscaleCoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::GetOneBlockRewardAndUnlock(UpscaleCoin::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                                 const UpscaleCoin::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::performTransferWithErrorTx(const std::array<int64_t, 5>& amounts, uint64_t fee) {
  std::vector<UpscaleCoin::WalletLegacyTransfer> trs;
  UpscaleCoin::WalletLegacyTransfer tr;
...
TEST_F(WalletLegacyApi, TransactionsAndTransfersAfterSend) {
  ...
  UpscaleCoin::WalletLegacyTransaction tx;
...
TEST_F(WalletLegacyApi, getTransactionSuccess) {
  ...
  EXPECT_EQ(tx.firstTransferId, UpscaleCoin::WALLET_LEGACY_INVALID_TRANSFER_ID);
...
TEST_F(WalletLegacyApi, wrongPassword) {
  ...
  EXPECT_EQ(result.value(), UpscaleCoin::error::WRONG_PASSWORD);
}
...
TEST_F(WalletLegacyApi, sendMoneyToMyself) {
  ...
  UpscaleCoin::TransactionId txId = TransferMoney(*alice, *alice, 100000000, 100);
  ASSERT_NE(txId, UpscaleCoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, checkBalanceAfterSend) {
  ...
  UpscaleCoin::TransactionId txId = TransferMoney(*alice, *alice, sendAmount, fee);
  ASSERT_NE(txId, UpscaleCoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, moneyInPoolDontAffectActualBalance) {
  ...
  UpscaleCoin::TransactionId txId = TransferMoney(*alice, *bob, sendAmount, fee);
  ASSERT_NE(txId, UpscaleCoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  generator.generateEmptyBlocks(10);
...
TEST_F(WalletLegacyApi, initWithKeys) {
  UpscaleCoin::AccountKeys accountKeys;
...
TEST_F(WalletLegacyApi, sendAfterFailedTransaction) {
  ...
  UpscaleCoin::TransactionId txId = TransferMoney(*alice, *alice, 100000, 100);
  ASSERT_NE(txId, UpscaleCoin::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  alice->shutdown();
}
...
TEST_F(WalletLegacyApi, outcommingExternalTransactionTotalAmount) {
  class ExternalTxChecker : public UpscaleCoin::IWalletLegacyObserver {
  public:
    ExternalTxChecker(UpscaleCoin::WalletLegacy& wallet) : wallet(wallet), totalAmount(std::numeric_limits<int64_t>::max()) {
    }

    virtual void externalTransactionCreated(UpscaleCoin::TransactionId transactionId) override {
      UpscaleCoin::WalletLegacyTransaction txInfo;
  ...
  UpscaleCoin::WalletLegacy wallet(m_currency, *aliceNode);
...
namespace {
class WalletSynchronizationProgressUpdatedObserver : public UpscaleCoin::IWalletLegacyObserver {
public:
...
TEST_F(WalletLegacyApi, shutdownDoesNotRemoveObservers) {
  ...
  UpscaleCoin::WalletHelper::IWalletRemoveObserverGuard observerGuard(*alice, observer);
...
namespace {
class WalletTransactionEventCounter : public UpscaleCoin::IWalletLegacyObserver {
public:
  virtual void externalTransactionCreated(UpscaleCoin::TransactionId /*transactionId*/) override {
    ++m_count;
  }

  virtual void transactionUpdated(UpscaleCoin::TransactionId /*transactionId*/) override {
    ++m_count;
  }
...
TEST_F(WalletLegacyApi, afterShutdownAndInitWalletDoesNotSendNotificationsRelatedToOldAddress) {
  ...
  UpscaleCoin::AccountKeys accountKeys1;
  ...
  std::vector<UpscaleCoin::WalletLegacyTransfer> transfers;
...
TEST_F(WalletLegacyApi, resetDoesNotChangeAccountKeys) {
  ...
  UpscaleCoin::AccountKeys expectedAccountKeys;
  alice->getAccountKeys(expectedAccountKeys);

  alice->reset();

  UpscaleCoin::AccountKeys actualAccountKeys;
...
void generateWallet(UpscaleCoin::IWalletLegacy& wallet, TrivialWalletObserver& observer, const std::string& pass) {
...
TEST_F(WalletLegacyApi, outdatedUnconfirmedTransactionDeletedOnNewBlock) {
  ...
  UpscaleCoin::Currency currency(UpscaleCoin::CurrencyBuilder(m_logger).mempoolTxLiveTime(TRANSACTION_MEMPOOL_TIME).currency());
  TestBlockchainGenerator blockchainGenerator(currency);
  INodeTrivialRefreshStub node(blockchainGenerator);
  UpscaleCoin::WalletLegacy wallet(currency, node);
  TrivialWalletObserver walletObserver;
  wallet.addObserver(&walletObserver);
  ...
  UpscaleCoin::WalletLegacyTransaction transaction;
  ASSERT_TRUE(wallet.getTransaction(id, transaction));
  EXPECT_EQ(UpscaleCoin::WalletLegacyTransactionState::Deleted, transaction.state);
...
TEST_F(WalletLegacyApi, walletLoadsNullSpendSecretKey) {
  ...
  Crypto::generate_keys(accountKeys.address.spendPublicKey, accountKeys.spendSecretKey);
  Crypto::generate_keys(accountKeys.address.viewPublicKey, accountKeys.viewSecretKey);
  accountKeys.spendSecretKey = UpscaleCoin::NULL_SECRET_KEY;
