...
#include "UpscaleCoinCore/TransactionPool.h"
...
class TransactionValidator : public UpscaleCoin::ITransactionValidator {
  virtual bool checkTransactionInputs(const UpscaleCoin::Transaction& tx, BlockInfo& maxUsedBlock) override {
    return true;
  }

  virtual bool checkTransactionInputs(const UpscaleCoin::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) override {
    return true;
  }

  virtual bool haveSpentKeyImages(const UpscaleCoin::Transaction& tx) override {
    return false;
  }
...
public:
  ...
  TestTransactionGenerator(const UpscaleCoin::Currency& currency, size_t ringSize) :
    m_currency(currency),
    m_ringSize(ringSize),
    m_miners(ringSize), 
...
namespace
{
  ...
  void GenerateTransaction(const UpscaleCoin::Currency& currency, Transaction& tx, uint64_t fee, size_t outputs) {
  ...
  public:
    ...
    TestPool(const UpscaleCoin::Currency& currency, Logging::ILogger& logger) :
  ...
  public:
    ...
    UpscaleCoin::RealTimeProvider m_time;
...
public:
  TxPool_FillBlockTemplate() :
    tx_pool() {
    currency = UpscaleCoin::CurrencyBuilder(logger).fusionTxMaxSize(TEST_FUSION_TX_MAX_SIZE).blockGrantedFullRewardZone(TEST_MEDIAN_SIZE).currency();
  }
