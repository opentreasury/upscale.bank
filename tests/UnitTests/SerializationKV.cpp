...
TEST(KVSerialize, Simple) {
  ...
  std::string buf = UpscaleCoin::storeToBinaryKeyValue(testData1);
  ASSERT_TRUE(UpscaleCoin::loadFromBinaryKeyValue(testData2, buf));
...
TEST(KVSerialize, BigCollection) {
  ...
  std::string buf = UpscaleCoin::storeToBinaryKeyValue(ts1);
  ASSERT_TRUE(UpscaleCoin::loadFromBinaryKeyValue(ts2, buf));
  EXPECT_EQ(ts1, ts2);
}
