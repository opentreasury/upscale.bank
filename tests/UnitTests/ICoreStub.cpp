...
#include "UpscaleCoinCore/IBlock.h"
...
ICoreStub::ICoreStub(const UpscaleCoin::Block& genesisBlock) :
    topHeight(0),
    globalIndicesResult(false),
    randomOutsResult(false),
    poolTxVerificationResult(true),
...
bool ICoreStub::addObserver(UpscaleCoin::ICoreObserver* observer) {
  return true;
}

bool ICoreStub::removeObserver(UpscaleCoin::ICoreObserver* observer) {
  return true;
}
...
bool ICoreStub::get_random_outs_for_amounts(const UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request& req,
    UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& res) {
  res = randomOuts;
...
UpscaleCoin::i_UpscaleCoin_protocol* ICoreStub::get_protocol() {
  return nullptr;
}

bool ICoreStub::handle_incoming_tx(UpscaleCoin::BinaryArray const& tx_blob, UpscaleCoin::tx_verification_context& tvc, bool keeped_by_block) {
  return true;
}
...
void ICoreStub::set_random_outs(const UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp, bool result) {
  randomOuts = resp;
  randomOutsResult = result;
}

std::vector<UpscaleCoin::Transaction> ICoreStub::getPoolTransactions() {
  return std::vector<UpscaleCoin::Transaction>();
}

bool ICoreStub::getPoolChanges(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
                               std::vector<UpscaleCoin::Transaction>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  ...
  for (const std::pair<Crypto::Hash, UpscaleCoin::Transaction>& poolEntry : transactionPool) {
...
bool ICoreStub::getPoolChangesLite(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
        std::vector<UpscaleCoin::TransactionPrefixInfo>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  std::vector<UpscaleCoin::Transaction> added;
  bool returnStatus = getPoolChanges(tailBlockId, knownTxsIds, added, deletedTxsIds);

  for (const auto& tx : added) {
    UpscaleCoin::TransactionPrefixInfo tpi;
    tpi.txPrefix = tx;
...
void ICoreStub::getPoolChanges(const std::vector<Crypto::Hash>& knownTxsIds, std::vector<UpscaleCoin::Transaction>& addedTxs,
                               std::vector<Crypto::Hash>& deletedTxsIds) {
}

bool ICoreStub::queryBlocks(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<UpscaleCoin::BlockFullInfo>& entries) {
  //stub
  return true;
}

bool ICoreStub::queryBlocksLite(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<UpscaleCoin::BlockShortInfo>& entries) {
  //stub
  return true;
}
...
size_t ICoreStub::addChain(const std::vector<const UpscaleCoin::IBlock*>& chain) {
  size_t blocksCounter = 0;
  for (const UpscaleCoin::IBlock* block : chain) {
    for (size_t txNumber = 0; txNumber < block->getTransactionCount(); ++txNumber) {
      const UpscaleCoin::Transaction& tx = block->getTransaction(txNumber);
      Crypto::Hash txHash = UpscaleCoin::NULL_HASH;
      size_t blobSize = 0;
...
Crypto::Hash ICoreStub::getBlockIdByHeight(uint32_t height) {
  ...
  if (iter == blockHashByHeightIndex.end()) {
    return UpscaleCoin::NULL_HASH;
  }
  return iter->second;
}

bool ICoreStub::getBlockByHash(const Crypto::Hash &h, UpscaleCoin::Block &blk) {
...
void ICoreStub::getTransactions(const std::vector<Crypto::Hash>& txs_ids, std::list<UpscaleCoin::Transaction>& txs, std::list<Crypto::Hash>& missed_txs, bool checkTxPool) {
...
bool ICoreStub::scanOutputkeysForIndices(const UpscaleCoin::KeyInput& txInToKey, std::list<std::pair<Crypto::Hash, size_t>>& outputReferences) {
  return true;
}

bool ICoreStub::getBlockDifficulty(uint32_t height, UpscaleCoin::difficulty_type& difficulty) {
  return true;
}
...
bool ICoreStub::getBlockContainingTx(const Crypto::Hash& txId, Crypto::Hash& blockId, uint32_t& blockHeight) {
  ...
  blockHeight = boost::get<UpscaleCoin::BaseInput>(blockIter->second.baseTransaction.inputs.front()).blockIndex;
  return true;
}

bool ICoreStub::getMultisigOutputReference(const UpscaleCoin::MultisignatureInput& txInMultisig, std::pair<Crypto::Hash, size_t>& outputReference) {
  return true;
}

void ICoreStub::addBlock(const UpscaleCoin::Block& block) {
  uint32_t height = boost::get<UpscaleCoin::BaseInput>(block.baseTransaction.inputs.front()).blockIndex;
  Crypto::Hash hash = UpscaleCoin::get_block_hash(block);
  if (height > topHeight) {
    topHeight = height;
    topId = hash;
  }
  ...
  blockHashByTxHashIndex.emplace(std::make_pair(UpscaleCoin::getObjectHash(block.baseTransaction), hash));
  for (auto txHash : block.transactionHashes) {
    blockHashByTxHashIndex.emplace(std::make_pair(txHash, hash));
  }
}

void ICoreStub::addTransaction(const UpscaleCoin::Transaction& tx) {
  Crypto::Hash hash = UpscaleCoin::getObjectHash(tx);
...
bool ICoreStub::getOrphanBlocksByHeight(uint32_t height, std::vector<UpscaleCoin::Block>& blocks) {
  return true;
}

bool ICoreStub::getBlocksByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit, std::vector<UpscaleCoin::Block>& blocks, uint32_t& blocksNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getPoolTransactionsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<UpscaleCoin::Transaction>& transactions, uint64_t& transactionsNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getTransactionsByPaymentId(const Crypto::Hash& paymentId, std::vector<UpscaleCoin::Transaction>& transactions) {
  return true;
}
...
std::unique_ptr<UpscaleCoin::IBlock> ICoreStub::getBlock(const Crypto::Hash& blockId) {
  return std::unique_ptr<UpscaleCoin::IBlock>(nullptr);
}

bool ICoreStub::handleIncomingTransaction(const UpscaleCoin::Transaction& tx, const Crypto::Hash& txHash, size_t blobSize, UpscaleCoin::tx_verification_context& tvc, bool keptByBlock) {
...
bool ICoreStub::addMessageQueue(UpscaleCoin::MessageQueue<UpscaleCoin::BlockchainMessage>& messageQueuePtr) {
  return true;
}

bool ICoreStub::removeMessageQueue(UpscaleCoin::MessageQueue<UpscaleCoin::BlockchainMessage>& messageQueuePtr) {
  return true;
}
