...
#include "UpscaleCoinProtocolHandler.h"
...
#include "UpscaleCoinCore/VerificationContext.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const UpscaleCoinConnectionContext& context) {
...
UpscaleCoinProtocolHandler::UpscaleCoinProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
...
size_t UpscaleCoinProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void UpscaleCoinProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void UpscaleCoinProtocolHandler::onConnectionOpened(UpscaleCoinConnectionContext& context) {
}

void UpscaleCoinProtocolHandler::onConnectionClosed(UpscaleCoinConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&IUpscaleCoinProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != UpscaleCoinConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&IUpscaleCoinProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void UpscaleCoinProtocolHandler::stop() {
  m_stop = true;
}

bool UpscaleCoinProtocolHandler::start_sync(UpscaleCoinConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == UpscaleCoinConnectionContext::state_synchronizing) {
...
bool UpscaleCoinProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void UpscaleCoinProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const UpscaleCoinConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t UpscaleCoinProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool UpscaleCoinProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, UpscaleCoinConnectionContext& context, bool is_inital) {
  if (context.m_state == UpscaleCoinConnectionContext::state_befor_handshake && !is_inital)
    return true;
  ...
  if (context.m_state == UpscaleCoinConnectionContext::state_synchronizing) {
    ...
    if (is_inital) {
      ...
      context.m_state = UpscaleCoinConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = UpscaleCoinConnectionContext::state_normal;
    }
  } else {
    ...
    context.m_state = UpscaleCoinConnectionContext::state_sync_required;
  }
...
bool UpscaleCoinProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, UpscaleCoinConnectionContext& ctx, Handler handler) {
...
int UpscaleCoinProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, UpscaleCoinConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &UpscaleCoinProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &UpscaleCoinProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &UpscaleCoinProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &UpscaleCoinProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &UpscaleCoinProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &UpscaleCoinProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &UpscaleCoinProtocolHandler::handleRequestTxPool)
...
int UpscaleCoinProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, UpscaleCoinConnectionContext& context) {
  ...
  if (context.m_state != UpscaleCoinConnectionContext::state_normal) {
    return 1;
  }

  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    UpscaleCoin::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
    ...
    if (tvc.m_verifivation_failed) {
      ...
      context.m_state = UpscaleCoinConnectionContext::state_shutdown;
      return 1;
    }
  }
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = UpscaleCoinConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int UpscaleCoinProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, UpscaleCoinConnectionContext& context) {
...
int UpscaleCoinProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, UpscaleCoinConnectionContext& context) {
...
int UpscaleCoinProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, UpscaleCoinConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(get_block_hash(b))) {
        context.m_state = UpscaleCoinConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == UpscaleCoinConnectionContext::state_synchronizing) {
...
int UpscaleCoinProtocolHandler::processObjects(UpscaleCoinConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
...
bool UpscaleCoinProtocolHandler::on_idle() {
  return m_core.on_idle();
}

int UpscaleCoinProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, UpscaleCoinConnectionContext& context) {
...
bool UpscaleCoinProtocolHandler::request_missing_objects(UpscaleCoinConnectionContext& context, bool check_having_blocks) {
...
bool UpscaleCoinProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&IUpscaleCoinProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int UpscaleCoinProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, UpscaleCoinConnectionContext& context) {
...
int UpscaleCoinProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     UpscaleCoinConnectionContext& context) {
...
void UpscaleCoinProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void UpscaleCoinProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_TRANSACTIONS::ID, buf);
}

void UpscaleCoinProtocolHandler::requestMissingPoolTransactions(const UpscaleCoinConnectionContext& context) {
...
void UpscaleCoinProtocolHandler::updateObservedHeight(uint32_t peerHeight, const UpscaleCoinConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void UpscaleCoinProtocolHandler::recalculateMaxObservedHeight(const UpscaleCoinConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const UpscaleCoinConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t UpscaleCoinProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool UpscaleCoinProtocolHandler::addObserver(IUpscaleCoinProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool UpscaleCoinProtocolHandler::removeObserver(IUpscaleCoinProtocolObserver* observer) {
