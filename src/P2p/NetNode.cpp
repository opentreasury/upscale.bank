  ...
  if (result != 0) {
    if (result == 1) {
      ...
      if (UPNP_AddPortMapping(urls.controlURL, igdData.first.servicetype, portString.str().c_str(),
        portString.str().c_str(), lanAddress, UpscaleCoin::UpscaleCoin_NAME, "TCP", 0, "0") != 0) {
...
namespace UpscaleCoin
{
  namespace
  {
    ...
    const command_line::arg_descriptor<std::string> arg_p2p_bind_port      = {"p2p-bind-port", "Port for p2p network protocol", std::to_string(UpscaleCoin::P2P_DEFAULT_PORT)};
  ...
  NodeServer::NodeServer(System::Dispatcher& dispatcher, UpscaleCoin::UpscaleCoinProtocolHandler& payload_handler, Logging::ILogger& log) :
    m_dispatcher(dispatcher),
    m_workingContextGroup(dispatcher),
    m_payload_handler(payload_handler),
    m_allow_local_ip(false),
    m_hide_my_port(false),
    m_network_id(UpscaleCoin_NETWORK),
    ...
    m_stop(false),
    // intervals
    // m_peer_handshake_idle_maker_interval(UpscaleCoin::P2P_DEFAULT_HANDSHAKE_INTERVAL),
  ...
  int NodeServer::handleCommand(const LevinProtocol::Command& cmd, BinaryArray& out, P2pConnectionContext& ctx, bool& handled) {
    ...
    if (cmd.isResponse && cmd.command == COMMAND_TIMED_SYNC::ID) {
      if (!handleTimedSyncResponse(cmd.buf, ctx)) {
        ...
        ctx.m_state = UpscaleCoinConnectionContext::state_shutdown;
      }
      return 0;
    }
...
#undef INVOKE_HANDLER
  ...
  bool NodeServer::init_config() {
    try {
      ...
      m_config.m_net_config.handshake_interval = UpscaleCoin::P2P_DEFAULT_HANDSHAKE_INTERVAL;
      m_config.m_net_config.connections_count = UpscaleCoin::P2P_DEFAULT_CONNECTIONS_COUNT;
      m_config.m_net_config.packet_max_size = UpscaleCoin::P2P_DEFAULT_PACKET_MAX_SIZE; //20 MB limit
      m_config.m_net_config.config_id = 0; // initial config
      m_config.m_net_config.connection_timeout = UpscaleCoin::P2P_DEFAULT_CONNECTION_TIMEOUT;
      m_config.m_net_config.ping_connection_timeout = UpscaleCoin::P2P_DEFAULT_PING_CONNECTION_TIMEOUT;
      m_config.m_net_config.send_peerlist_sz = UpscaleCoin::P2P_DEFAULT_PEERS_IN_HANDSHAKE;
  ...
  //----------------------------------------------------------------------------------- 
  void NodeServer::for_each_connection(std::function<void(UpscaleCoinConnectionContext&, PeerIdType)> f)
  {
  ...
  bool NodeServer::init(const NetNodeConfig& config) {
    if (!config.getTestnet()) {
      for (auto seed : UpscaleCoin::SEED_NODES) {
    ...
    // m_net_server.get_config_object().m_invoke_timeout = UpscaleCoin::P2P_DEFAULT_INVOKE_TIMEOUT;
  ...
  UpscaleCoin::UpscaleCoinProtocolHandler& NodeServer::get_payload_object()
  {
  ...
  //----------------------------------------------------------------------------------- 
  bool NodeServer::handshake(UpscaleCoin::LevinProtocol& proto, P2pConnectionContext& context, bool just_take_peerlist) {
  ...
  bool NodeServer::timedSync() {
    ...
    forEachConnection([&](P2pConnectionContext& conn) {
      if (conn.peerId && 
          (conn.m_state == UpscaleCoinConnectionContext::state_normal ||
           conn.m_state == UpscaleCoinConnectionContext::state_idle)) {
    ...
    try {
      ...
      try {
        System::Context<bool> handshakeContext(m_dispatcher, [&] {
          UpscaleCoin::LevinProtocol proto(ctx.connection);
  ...
  bool NodeServer::connections_maker()
  {
    ...
    size_t expected_white_connections = (m_config.m_net_config.connections_count * UpscaleCoin::P2P_DEFAULT_WHITELIST_CONNECTIONS_PERCENT) / 100;
  ...
  bool NodeServer::handle_remote_peerlist(const std::list<PeerlistEntry>& peerlist, time_t local_time, const UpscaleCoinConnectionContext& context)
  {
    int64_t delta = 0;
  ...
  bool NodeServer::check_trust(const proof_of_trust &tr) {
    ...
    Crypto::PublicKey pk;
    Common::podFromHex(UpscaleCoin::P2P_STAT_TRUSTED_PUB_KEY, pk);
  ...
  void NodeServer::relay_notify_to_all(int command, const BinaryArray& data_buff, const net_connection_id* excludeConnection) {
    ...
    forEachConnection([&](P2pConnectionContext& conn) {
      if (conn.peerId && conn.m_connection_id != excludeId &&
          (conn.m_state == UpscaleCoinConnectionContext::state_normal ||
           conn.m_state == UpscaleCoinConnectionContext::state_synchronizing)) {
  ...
  //-----------------------------------------------------------------------------------
  bool NodeServer::invoke_notify_to_peer(int command, const BinaryArray& buffer, const UpscaleCoinConnectionContext& context) {
  ...
  void NodeServer::connectionHandler(const boost::uuids::uuid& connectionId, P2pConnectionContext& ctx) {
    ...
    System::Context<> context(m_dispatcher, [this, &connectionId, &ctx] {
      ...
      try {
        ...
        for (;;) {
          if (ctx.m_state == UpscaleCoinConnectionContext::state_sync_required) {
            ctx.m_state = UpscaleCoinConnectionContext::state_synchronizing;
            m_payload_handler.start_sync(ctx);
          } else if (ctx.m_state == UpscaleCoinConnectionContext::state_pool_sync_required) {
            ctx.m_state = UpscaleCoinConnectionContext::state_normal;
          ...
          if (ctx.m_state == UpscaleCoinConnectionContext::state_shutdown) {
            break;
          }
        }
      } catch (System::InterruptedException&) {
