...
const UpscaleCoin::Currency PaymentGateService::getCurrency() {
...
void PaymentGateService::runInProcess(Logging::LoggerRef& log) {
  ...
  UpscaleCoin::core core(currency, NULL, logger);

  UpscaleCoin::UpscaleCoinProtocolHandler protocol(currency, *dispatcher, core, NULL, logger);
  UpscaleCoin::NodeServer p2pNode(*dispatcher, protocol, logger);

  protocol.set_p2p_endpoint(&p2pNode);
  core.set_UpscaleCoin_protocol(&protocol);
  ...
  std::unique_ptr<UpscaleCoin::INode> node(new UpscaleCoin::InProcessNode(core, protocol));
...
void PaymentGateService::runRpcProxy(Logging::LoggerRef& log) {
  ...
  std::unique_ptr<UpscaleCoin::INode> node(
...
void PaymentGateService::runWalletService(const UpscaleCoin::Currency& currency, UpscaleCoin::INode& node) {
  PaymentService::WalletConfiguration walletConfiguration{
    config.gateConfiguration.containerFile,
    config.gateConfiguration.containerPassword
  };

  std::unique_ptr<UpscaleCoin::IWallet> wallet (WalletFactory::createWallet(currency, node, *dispatcher));
