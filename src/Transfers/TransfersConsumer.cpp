...
std::vector<Crypto::Hash> getBlockHashes(const UpscaleCoin::CompleteBlock* blocks, size_t count) {
...
TransfersConsumer::TransfersConsumer(const UpscaleCoin::Currency& currency, INode& node, const SecretKey& viewSecret) :
...
std::error_code TransfersConsumer::onPoolUpdated(const std::vector<std::unique_ptr<ITransactionReader>>& addedTransactions, const std::vector<Hash>& deletedTransactions) {
  ...
  std::error_code processingError;
  for (auto& UpscaleCoinTransaction : addedTransactions) {
    m_poolTxs.emplace(UpscaleCoinTransaction->getTransactionHash());
    processingError = processTransaction(unconfirmedBlockInfo, *UpscaleCoinTransaction.get());
    if (processingError) {
...
std::error_code createTransfers(
  ...
  for (auto idx : outputs) {
    ...
    if (outType == TransactionTypes::OutputType::Key) {
      ...
      UpscaleCoin::KeyPair in_ephemeral;
      UpscaleCoin::generate_key_image_helper(
        account,
        txPubKey,
        idx,
        in_ephemeral,
