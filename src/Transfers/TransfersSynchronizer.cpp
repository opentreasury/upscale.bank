...
TransfersSyncronizer::TransfersSyncronizer(const UpscaleCoin::Currency& currency, IBlockchainSynchronizer& sync, INode& node) :
...
void TransfersSyncronizer::save(std::ostream& os) {
  ...
  UpscaleCoin::BinaryOutputStreamSerializer s(stream);
...
void TransfersSyncronizer::load(std::istream& is) {
  ...
  UpscaleCoin::BinaryInputStreamSerializer s(inputStream);
  uint32_t version = 0;
