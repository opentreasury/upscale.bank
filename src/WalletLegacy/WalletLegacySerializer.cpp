...
WalletLegacySerializer::WalletLegacySerializer(UpscaleCoin::AccountBase& account, WalletUserTransactionsCache& transactionsCache) :
  account(account),
...
void WalletLegacySerializer::serialize(std::ostream& stream, const std::string& password, bool saveDetailed, const std::string& cache) {
  ...
  UpscaleCoin::BinaryOutputStreamSerializer serializer(plainStream);
  saveKeys(serializer);
...
void WalletLegacySerializer::saveKeys(UpscaleCoin::ISerializer& serializer) {
  UpscaleCoin::KeysStorage keys;
  UpscaleCoin::AccountKeys acc = account.getAccountKeys();
...
void WalletLegacySerializer::deserialize(std::istream& stream, const std::string& password, std::string& cache) {
  ...
  UpscaleCoin::BinaryInputStreamSerializer serializerEncrypted(stdStream);
...
void WalletLegacySerializer::loadKeys(UpscaleCoin::ISerializer& serializer) {
  UpscaleCoin::KeysStorage keys;

  keys.serialize(serializer, "keys");

  UpscaleCoin::AccountKeys acc;
