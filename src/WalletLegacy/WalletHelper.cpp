...
std::error_code walletSaveWrapper(UpscaleCoin::IWalletLegacy& wallet, std::ofstream& file, bool saveDetailes, bool saveCache) {
  UpscaleCoin::WalletHelper::SaveWalletResultObserver o;
...
void WalletHelper::SendCompleteResultObserver::sendTransactionCompleted(UpscaleCoin::TransactionId transactionId, std::error_code result) {
  std::lock_guard<std::mutex> lock(m_mutex);
  m_finishedTransactions[transactionId] = result;
  m_condition.notify_one();
}

std::error_code WalletHelper::SendCompleteResultObserver::wait(UpscaleCoin::TransactionId transactionId) {
...
WalletHelper::IWalletRemoveObserverGuard::IWalletRemoveObserverGuard(UpscaleCoin::IWalletLegacy& wallet, UpscaleCoin::IWalletLegacyObserver& observer) :
  m_wallet(wallet),
  m_observer(observer),
  m_removed(false) {
...
void WalletHelper::storeWallet(UpscaleCoin::IWalletLegacy& wallet, const std::string& walletFilename) {
