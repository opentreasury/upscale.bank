...
BlockchainExplorerDataBuilder::BlockchainExplorerDataBuilder(UpscaleCoin::ICore& core, UpscaleCoin::IUpscaleCoinProtocolQuery& protocol) :
core(core),
protocol(protocol) {
}
...
bool BlockchainExplorerDataBuilder::fillBlockDetails(const Block &block, BlockDetails& blockDetails) {
  ...
  if (!core.getBackwardBlocksSizes(blockDetails.height, blocksSizes, parameters::UpscaleCoin_REWARD_BLOCKS_WINDOW)) {
    return false;
  }
