...
namespace {
  ...
  static bool print_as_json(const T& obj) {
    std::cout << UpscaleCoin::storeToJson(obj) << ENDL;
    return true;
  }
}


DaemonCommandsHandler::DaemonCommandsHandler(UpscaleCoin::core& core, UpscaleCoin::NodeServer& srv, Logging::LoggerManager& log) :
...
{
  std::stringstream ss;
  ss << UpscaleCoin::UpscaleCoin_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
  ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::start_mining(const std::vector<std::string> &args) {
  ...
  UpscaleCoin::AccountPublicAddress adr;
