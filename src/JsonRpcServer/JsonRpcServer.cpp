...
void JsonRpcServer::processRequest(const UpscaleCoin::HttpRequest& req, UpscaleCoin::HttpResponse& resp) {
  try {
    ...
    if (req.getUrl() == "/json_rpc") {
      ...
      try {
        ...
        resp.setStatus(UpscaleCoin::HttpResponse::STATUS_200);
      ...
      resp.setStatus(UpscaleCoin::HttpResponse::STATUS_404);
      return;
    }
  } catch (std::exception& e) {
    logger(Logging::WARNING) << "Error while processing http request: " << e.what();
    resp.setStatus(UpscaleCoin::HttpResponse::STATUS_500);
  }
}
