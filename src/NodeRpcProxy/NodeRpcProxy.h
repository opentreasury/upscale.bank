...
class NodeRpcProxy : public UpscaleCoin::INode {
public:
  ...
  virtual bool addObserver(UpscaleCoin::INodeRpcProxyObserver* observer);
  virtual bool removeObserver(UpscaleCoin::INodeRpcProxyObserver* observer);
...
private:
  ...
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>& knownBlockIds,
    std::vector<UpscaleCoin::block_complete_entry>& newBlocks, uint32_t& startHeight);
...
private:
  ...
  Tools::ObserverManager<UpscaleCoin::INodeObserver> m_observerManager;
  Tools::ObserverManager<UpscaleCoin::INodeRpcProxyObserver> m_rpcProxyObserverManager;
