...
class PaymentServiceJsonRpcServer : public UpscaleCoin::JsonRpcServer {
public:
...
private:
  ...
  HandlerFunction jsonHandler(RequestHandler handler) {
    return [handler] (const Common::JsonValue& jsonRpcParams, Common::JsonValue& jsonResponse) mutable {
      ...
      try {
        UpscaleCoin::JsonInputValueSerializer inputSerializer(const_cast<Common::JsonValue&>(jsonRpcParams));
      ...
      UpscaleCoin::JsonOutputStreamSerializer outputSerializer;
