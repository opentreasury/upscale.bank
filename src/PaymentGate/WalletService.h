...
class WalletService {
public:
  WalletService(const UpscaleCoin::Currency& currency, System::Dispatcher& sys, UpscaleCoin::INode& node, UpscaleCoin::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger);
...
private:
  ...
  std::vector<UpscaleCoin::TransactionsInBlockInfo> getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const;
  std::vector<UpscaleCoin::TransactionsInBlockInfo> getTransactions(uint32_t firstBlockIndex, size_t blockCount) const;
  ...
  UpscaleCoin::IWallet& wallet;
  UpscaleCoin::INode& node;
