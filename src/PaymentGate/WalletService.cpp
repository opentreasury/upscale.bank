...
Crypto::Hash parsePaymentId(const std::string& paymentIdStr) {
  if (!checkPaymentId(paymentIdStr)) {
    throw std::system_error(make_error_code(UpscaleCoin::error::WalletServiceErrorCode::WRONG_PAYMENT_ID_FORMAT));
  }
...
bool getPaymentIdFromExtra(const std::string& binaryString, Crypto::Hash& paymentId) {
  return UpscaleCoin::getPaymentIdFromTxExtra(Common::asBinaryArray(binaryString), paymentId);
}
...
struct TransactionsInBlockInfoFilter {
  ...
  bool checkTransaction(const UpscaleCoin::WalletTransactionWithTransfers& transaction) const {
    if (havePaymentId) {
    ...
    for (const UpscaleCoin::WalletTransfer& transfer: transaction.transfers) {
...
void addPaymentIdToExtra(const std::string& paymentId, std::string& extra) {
  ...
  if (!UpscaleCoin::createTxExtraWithPaymentId(paymentId, extraVector)) {
...
Crypto::Hash parseHash(const std::string& hashString, Logging::LoggerRef logger) {
  ...
  if (!Common::podFromHex(hashString, hash)) {
    ...
    throw std::system_error(make_error_code(UpscaleCoin::error::WalletServiceErrorCode::WRONG_HASH_FORMAT));
  }

  return hash;
}

std::vector<UpscaleCoin::TransactionsInBlockInfo> filterTransactions(
  const std::vector<UpscaleCoin::TransactionsInBlockInfo>& blocks,
  const TransactionsInBlockInfoFilter& filter) {

  std::vector<UpscaleCoin::TransactionsInBlockInfo> result;

  for (const auto& block: blocks) {
    UpscaleCoin::TransactionsInBlockInfo item;
    item.blockHash = block.blockHash;

    for (const auto& transaction: block.transactions) {
      if (transaction.transaction.state != UpscaleCoin::WalletTransactionState::DELETED && filter.checkTransaction(transaction)) {
...
PaymentService::TransactionRpcInfo convertTransactionWithTransfersToTransactionRpcInfo(
  const UpscaleCoin::WalletTransactionWithTransfers& transactionWithTransfers) {
  ...
  for (const UpscaleCoin::WalletTransfer& transfer: transactionWithTransfers.transfers) {
...
std::vector<PaymentService::TransactionsInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionsInBlockRpcInfo(
  ...
  for (const auto& block: blocks) {
    ...
    for (const UpscaleCoin::WalletTransactionWithTransfers& transactionWithTransfers: block.transactions) {
...
std::vector<PaymentService::TransactionHashesInBlockRpcInfo> convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(
  ...
  for (const UpscaleCoin::TransactionsInBlockInfo& block: blocks) {
    PaymentService::TransactionHashesInBlockRpcInfo item;
    item.blockHash = Common::podToHex(block.blockHash);

    for (const UpscaleCoin::WalletTransactionWithTransfers& transaction: block.transactions) {
...
void validateAddresses(const std::vector<std::string>& addresses, const UpscaleCoin::Currency& currency, Logging::LoggerRef logger) {
...
std::vector<UpscaleCoin::WalletOrder> convertWalletRpcOrdersToWalletOrders(const std::vector<PaymentService::WalletRpcOrder>& orders) {
  std::vector<UpscaleCoin::WalletOrder> result;
  result.reserve(orders.size());

  for (const auto& order: orders) {
    result.emplace_back(UpscaleCoin::WalletOrder {order.address, order.amount});
  }
...
void saveWallet(UpscaleCoin::IWallet& wallet, std::fstream& walletFile, bool saveDetailed = true, bool saveCache = true) {
  wallet.save(walletFile, saveDetailed, saveCache);
  walletFile.flush();
}

void secureSaveWallet(UpscaleCoin::IWallet& wallet, const std::string& path, bool saveDetailed = true, bool saveCache = true) {
...
void generateNewWallet(const UpscaleCoin::Currency &currency, const WalletConfiguration &conf, Logging::ILogger& logger, System::Dispatcher& dispatcher) {
  Logging::LoggerRef log(logger, "generateNewWallet");

  UpscaleCoin::INode* nodeStub = NodeFactory::createNodeStub();
  std::unique_ptr<UpscaleCoin::INode> nodeGuard(nodeStub);

  UpscaleCoin::IWallet* wallet = WalletFactory::createWallet(currency, *nodeStub, dispatcher);
  std::unique_ptr<UpscaleCoin::IWallet> walletGuard(wallet);
...
void importLegacyKeys(const std::string &legacyKeysFile, const WalletConfiguration &conf) {
  ...
  UpscaleCoin::importLegacyKeys(legacyKeysFile, conf.walletPassword, archive);
...
WalletService::WalletService(const UpscaleCoin::Currency& currency, System::Dispatcher& sys, UpscaleCoin::INode& node,
  UpscaleCoin::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger) :
    currency(currency),
    wallet(wallet),
    node(node),
    config(conf),
...
std::error_code WalletService::resetWallet() {
  try {
    ...
    if (!inited) {
      ...
      return make_error_code(UpscaleCoin::error::NOT_INITIALIZED);
    }
    ...
    logger(Logging::WARNING) << "Error while reseting wallet: " << x.what();
    return make_error_code(UpscaleCoin::error::INTERNAL_WALLET_ERROR);
  }
...
std::error_code WalletService::replaceWithNewWallet(const std::string& viewSecretKeyText) {
  try {
    ...
    if (!Common::podFromHex(viewSecretKeyText, viewSecretKey)) {
      ...
      return make_error_code(UpscaleCoin::error::WalletServiceErrorCode::WRONG_KEY_FORMAT);
    }
...
std::error_code WalletService::getSpendkeys(const std::string& address, std::string& publicSpendKeyText, std::string& secretSpendKeyText) {
  try {
    ...
    UpscaleCoin::KeyPair key = wallet.getAddressSpendKey(address);
...
std::error_code WalletService::getViewKey(std::string& viewSecretKey) {
  try {
    System::EventLock lk(readyEvent);
    UpscaleCoin::KeyPair viewKey = wallet.getViewKey();
...
std::error_code WalletService::getTransaction(const std::string& transactionHash, TransactionRpcInfo& transaction) {
  try {
    ...
    UpscaleCoin::WalletTransactionWithTransfers transactionWithTransfers = wallet.getTransaction(hash);

    if (transactionWithTransfers.transaction.state == UpscaleCoin::WalletTransactionState::DELETED) {
      logger(Logging::WARNING) << "Transaction " << transactionHash << " is deleted";
      return make_error_code(UpscaleCoin::error::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::sendTransaction(const SendTransaction::Request& request, std::string& transactionHash) {
  try {
    ...
    UpscaleCoin::TransactionParameters sendParams;
...
std::error_code WalletService::deleteDelayedTransaction(const std::string& transactionHash) {
  try {
    ...
    if (idIt == transactionIdIndex.end()) {
      return make_error_code(UpscaleCoin::error::WalletServiceErrorCode::OBJECT_NOT_FOUND);
    }
...
std::error_code WalletService::getUnconfirmedTransactionHashes(const std::vector<std::string>& addresses, std::vector<std::string>& transactionHashes) {
  try {
    ...
    std::vector<UpscaleCoin::WalletTransactionWithTransfers> transactions = wallet.getUnconfirmedTransactions();
...
void WalletService::refresh() {
  try {
    ...
    for (;;) {
      ...
      if (event.type == UpscaleCoin::TRANSACTION_CREATED) {
...
std::vector<UpscaleCoin::TransactionsInBlockInfo> WalletService::getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const {
  std::vector<UpscaleCoin::TransactionsInBlockInfo> result = wallet.getTransactions(blockHash, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(UpscaleCoin::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }

  return result;
}

std::vector<UpscaleCoin::TransactionsInBlockInfo> WalletService::getTransactions(uint32_t firstBlockIndex, size_t blockCount) const {
  std::vector<UpscaleCoin::TransactionsInBlockInfo> result = wallet.getTransactions(firstBlockIndex, blockCount);
  if (result.empty()) {
    throw std::system_error(make_error_code(UpscaleCoin::error::WalletServiceErrorCode::OBJECT_NOT_FOUND));
  }
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(const Crypto::Hash& blockHash, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<UpscaleCoin::TransactionsInBlockInfo> allTransactions = getTransactions(blockHash, blockCount);
  std::vector<UpscaleCoin::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
...
std::vector<TransactionHashesInBlockRpcInfo> WalletService::getRpcTransactionHashes(uint32_t firstBlockIndex, size_t blockCount, const TransactionsInBlockInfoFilter& filter) const {
  std::vector<UpscaleCoin::TransactionsInBlockInfo> allTransactions = getTransactions(firstBlockIndex, blockCount);
  std::vector<UpscaleCoin::TransactionsInBlockInfo> filteredTransactions = filterTransactions(allTransactions, filter);
  return convertTransactionsInBlockInfoToTransactionHashesInBlockRpcInfo(filteredTransactions);
}
