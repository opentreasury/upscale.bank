...
Crypto::Hash BlockchainMonitor::requestLastBlockHash() {
  ...
  try {
    UpscaleCoin::HttpClient client(m_dispatcher, m_daemonHost, m_daemonPort);

    UpscaleCoin::COMMAND_RPC_GET_LAST_BLOCK_HEADER::request request;
    UpscaleCoin::COMMAND_RPC_GET_LAST_BLOCK_HEADER::response response;

    System::EventLock lk(m_httpEvent);
    UpscaleCoin::JsonRpc::invokeJsonRpcCommand(client, "getlastblockheader", request, response);
