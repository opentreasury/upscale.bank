...
class InProcessNode : public INode, public UpscaleCoin::IUpscaleCoinProtocolObserver, public UpscaleCoin::ICoreObserver {
public:
  InProcessNode(UpscaleCoin::ICore& core, UpscaleCoin::IUpscaleCoinProtocolQuery& protocol);
...
private:
  ...
  void getNewBlocksAsync(std::vector<Crypto::Hash>& knownBlockIds, std::vector<UpscaleCoin::block_complete_entry>& newBlocks, uint32_t& startHeight, const Callback& callback);
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<UpscaleCoin::block_complete_entry>& newBlocks, uint32_t& startHeight);
  ...
  std::error_code doGetRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount,
      std::vector<UpscaleCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result);

  void relayTransactionAsync(const UpscaleCoin::Transaction& transaction, const Callback& callback);
  std::error_code doRelayTransaction(const UpscaleCoin::Transaction& transaction);
