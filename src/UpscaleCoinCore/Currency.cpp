...
#include "Account.h"
#include "UpscaleCoinBasicImpl.h"
...
bool Currency::parseAccountAddressString(const std::string& str, AccountPublicAddress& addr) const {
  uint64_t prefix;
  if (!UpscaleCoin::parseAccountAddressString(prefix, addr, str)) {
    return false;
  }
...
CurrencyBuilder::CurrencyBuilder(Logging::ILogger& log) : m_currency(log) {
  maxBlockNumber(parameters::UPSCALECOIN_MAX_BLOCK_NUMBER);
  maxBlockBlobSize(parameters::UpscaleCoin_MAX_BLOCK_BLOB_SIZE);
  maxTxSize(parameters::UpscaleCoin_MAX_TX_SIZE);
  publicAddressBase58Prefix(parameters::UpscaleCoin_PUBLIC_ADDRESS_BASE58_PREFIX);
  minedMoneyUnlockWindow(parameters::UpscaleCoin_MINED_MONEY_UNLOCK_WINDOW);

  timestampCheckWindow(parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW);
  blockFutureTimeLimit(parameters::UpscaleCoin_BLOCK_FUTURE_TIME_LIMIT);

  moneySupply(parameters::MONEY_SUPPLY);
  emissionSpeedFactor(parameters::EMISSION_SPEED_FACTOR);

  rewardBlocksWindow(parameters::UpscaleCoin_REWARD_BLOCKS_WINDOW);
  blockGrantedFullRewardZone(parameters::UpscaleCoin_BLOCK_GRANTED_FULL_REWARD_ZONE);
  minerTxBlobReservedSize(parameters::UpscaleCoin_COINBASE_BLOB_RESERVED_SIZE);

  numberOfDecimalPlaces(parameters::UpscaleCoin_DISPLAY_DECIMAL_POINT);
  ...
  lockedTxAllowedDeltaSeconds(parameters::UpscaleCoin_LOCKED_TX_ALLOWED_DELTA_SECONDS);
  lockedTxAllowedDeltaBlocks(parameters::UpscaleCoin_LOCKED_TX_ALLOWED_DELTA_BLOCKS);

  mempoolTxLiveTime(parameters::UpscaleCoin_MEMPOOL_TX_LIVETIME);
  mempoolTxFromAltBlockLiveTime(parameters::UpscaleCoin_MEMPOOL_TX_FROM_ALT_BLOCK_LIVETIME);
  numberOfPeriodsToForgetTxDeletedFromPool(parameters::UpscaleCoin_NUMBER_OF_PERIODS_TO_FORGET_TX_DELETED_FROM_POOL);

  fusionTxMaxSize(parameters::FUSION_TX_MAX_SIZE);
  fusionTxMinInputCount(parameters::FUSION_TX_MIN_INPUT_COUNT);
  fusionTxMinInOutCountRatio(parameters::FUSION_TX_MIN_IN_OUT_COUNT_RATIO);

  blocksFileName(parameters::UpscaleCoin_BLOCKS_FILENAME);
  blocksCacheFileName(parameters::UpscaleCoin_BLOCKSCACHE_FILENAME);
  blockIndexesFileName(parameters::UpscaleCoin_BLOCKINDEXES_FILENAME);
  txPoolFileName(parameters::UpscaleCoin_POOLDATA_FILENAME);
  blockchinIndicesFileName(parameters::UpscaleCoin_BLOCKCHAIN_INDICES_FILENAME);
...
Transaction CurrencyBuilder::generateGenesisTransaction() {
  ...
  UpscaleCoin::AccountPublicAddress ac = boost::value_initialized<UpscaleCoin::AccountPublicAddress>();
