...
#include "UpscaleCoinSerialization.h"
...
#include "UpscaleCoinFormatUtils.h"
#include "UpscaleCoinTools.h"
...
struct BinaryVariantTagGetter: boost::static_visitor<uint8_t> {
  uint8_t operator()(const UpscaleCoin::BaseInput) { return  0xff; }
  uint8_t operator()(const UpscaleCoin::KeyInput) { return  0x2; }
  uint8_t operator()(const UpscaleCoin::MultisignatureInput) { return  0x3; }
  uint8_t operator()(const UpscaleCoin::KeyOutput) { return  0x2; }
  uint8_t operator()(const UpscaleCoin::MultisignatureOutput) { return  0x3; }
  uint8_t operator()(const UpscaleCoin::Transaction) { return  0xcc; }
  uint8_t operator()(const UpscaleCoin::Block) { return  0xbb; }
};

struct VariantSerializer : boost::static_visitor<> {
  VariantSerializer(UpscaleCoin::ISerializer& serializer, const std::string& name) : s(serializer), name(name) {}

  template <typename T>
  void operator() (T& param) { s(param, name); }

  UpscaleCoin::ISerializer& s;
  std::string name;
};

void getVariantValue(UpscaleCoin::ISerializer& serializer, uint8_t tag, UpscaleCoin::TransactionInput& in) {
  switch(tag) {
  case 0xff: {
    UpscaleCoin::BaseInput v;
    serializer(v, "value");
    in = v;
    break;
  }
  case 0x2: {
    UpscaleCoin::KeyInput v;
    serializer(v, "value");
    in = v;
    break;
  }
  case 0x3: {
    UpscaleCoin::MultisignatureInput v;
    serializer(v, "value");
    in = v;
    break;
  }
...
void getVariantValue(UpscaleCoin::ISerializer& serializer, uint8_t tag, UpscaleCoin::TransactionOutputTarget& out) {
  switch(tag) {
  case 0x2: {
    UpscaleCoin::KeyOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
  case 0x3: {
    UpscaleCoin::MultisignatureOutput v;
    serializer(v, "data");
    out = v;
    break;
  }
...
template <typename T>
bool serializePod(T& v, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
  return serializer.binary(&v, sizeof(v), name);
}

bool serializeVarintVector(std::vector<uint32_t>& vector, UpscaleCoin::ISerializer& serializer, Common::StringView name) {
...
bool serialize(PublicKey& pubKey, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
  return serializePod(pubKey, name, serializer);
}

bool serialize(SecretKey& secKey, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
  return serializePod(secKey, name, serializer);
}

bool serialize(Hash& h, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
  return serializePod(h, name, serializer);
}

bool serialize(KeyImage& keyImage, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
  return serializePod(keyImage, name, serializer);
}

bool serialize(chacha8_iv& chacha, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
  return serializePod(chacha, name, serializer);
}

bool serialize(Signature& sig, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
  return serializePod(sig, name, serializer);
}

bool serialize(EllipticCurveScalar& ecScalar, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
  return serializePod(ecScalar, name, serializer);
}

bool serialize(EllipticCurvePoint& ecPoint, Common::StringView name, UpscaleCoin::ISerializer& serializer) {
