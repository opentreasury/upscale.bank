...
namespace UpscaleCoin {
  ...
  public:
    ...
    TransactionImpl(const UpscaleCoin::Transaction& tx);
  ...
  std::unique_ptr<ITransaction> createTransaction(const UpscaleCoin::Transaction& tx) {
    return std::unique_ptr<ITransaction>(new TransactionImpl(tx));
  }

  TransactionImpl::TransactionImpl() {   
    UpscaleCoin::KeyPair txKeys(UpscaleCoin::generateKeyPair());
  ...
  TransactionImpl::TransactionImpl(const UpscaleCoin::Transaction& tx) : transaction(tx) {
    extra.parse(transaction.extra);
  }
  ...
  bool TransactionImpl::findOutputsToAccount(const AccountPublicAddress& addr, const SecretKey& viewSecretKey, std::vector<uint32_t>& out, uint64_t& amount) const {
    return ::UpscaleCoin::findOutputsToAccount(transaction, addr, viewSecretKey, out, amount);
  }
