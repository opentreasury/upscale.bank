...
protected:
  ...
  struct ReceiverAmounts {
    UpscaleCoin::AccountPublicAddress receiver;
  ...
  void updateBalance(UpscaleCoin::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(UpscaleCoin::ITransfersContainer* container) const;

  UpscaleCoin::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const UpscaleCoin::AccountPublicAddress& changeDestinationAddress,
  ...
  std::unique_ptr<UpscaleCoin::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp);

  void sendTransaction(const UpscaleCoin::Transaction& UpscaleCoinTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const UpscaleCoin::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, UpscaleCoin::ITransfersContainer* container);
  ...
  UpscaleCoin::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;
