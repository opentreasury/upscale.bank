...
struct WalletTransactionDto {
  ...
  WalletTransactionDto(const UpscaleCoin::WalletTransaction& wallet) {
    state = wallet.state;
  ...
  UpscaleCoin::WalletTransactionState state;
  uint64_t timestamp;
  uint32_t blockHeight;
  Hash hash;
  int64_t totalAmount;
...
struct WalletTransferDto {
  ...
  WalletTransferDto(const UpscaleCoin::WalletTransfer& tr, uint32_t version) : WalletTransferDto(version) {
    address = tr.address;
    amount = tr.amount;
...
void serialize(WalletRecordDto& value, UpscaleCoin::ISerializer& serializer) {
...
void serialize(ObsoleteSpentOutputDto& value, UpscaleCoin::ISerializer& serializer) {
...
void serialize(ObsoleteChangeDto& value, UpscaleCoin::ISerializer& serializer) {
  serializer(value.txHash, "transaction_hash");
  serializer(value.amount, "amount");
}

void serialize(UnlockTransactionJobDto& value, UpscaleCoin::ISerializer& serializer) {
  serializer(value.blockHeight, "block_height");
  serializer(value.transactionHash, "transaction_hash");
  serializer(value.walletIndex, "wallet_index");
}

void serialize(WalletTransactionDto& value, UpscaleCoin::ISerializer& serializer) {
  typedef std::underlying_type<UpscaleCoin::WalletTransactionState>::type StateType;

  StateType state = static_cast<StateType>(value.state);
  serializer(state, "state");
  value.state = static_cast<UpscaleCoin::WalletTransactionState>(state);

  serializer(value.timestamp, "timestamp");
  UpscaleCoin::serializeBlockHeight(serializer, value.blockHeight, "block_height");
...
void serialize(WalletTransferDto& value, UpscaleCoin::ISerializer& serializer) {
...
std::string serialize(Object& obj, const std::string& name) {
  ...
  UpscaleCoin::BinaryOutputStreamSerializer s(output);
...
std::string encrypt(const std::string& plain, UpscaleCoin::CryptoContext& cryptoContext) {
  std::string cipher;
...
void addToStream(const std::string& cipher, const std::string& name, Common::IOutputStream& destination) {
  UpscaleCoin::BinaryOutputStreamSerializer s(destination);
  s(const_cast<std::string& >(cipher), name);
}

template<typename Object>
void serializeEncrypted(Object& obj, const std::string& name, UpscaleCoin::CryptoContext& cryptoContext, Common::IOutputStream& destination) {
...
std::string readCipher(Common::IInputStream& source, const std::string& name) {
  std::string cipher;
  UpscaleCoin::BinaryInputStreamSerializer s(source);
  s(cipher, name);

  return cipher;
}

std::string decrypt(const std::string& cipher, UpscaleCoin::CryptoContext& cryptoContext) {
  std::string plain;
...
void deserialize(Object& obj, const std::string& name, const std::string& plain) {
  ...
  UpscaleCoin::BinaryInputStreamSerializer s(stream);
  s(obj, Common::StringView(name));
}

template<typename Object>
void deserializeEncrypted(Object& obj, const std::string& name, UpscaleCoin::CryptoContext& cryptoContext, Common::IInputStream& source) {
...
UpscaleCoin::WalletTransaction convert(const UpscaleCoin::WalletLegacyTransaction& tx) {
  UpscaleCoin::WalletTransaction mtx;

  mtx.state = UpscaleCoin::WalletTransactionState::SUCCEEDED;
...
UpscaleCoin::WalletTransfer convert(const UpscaleCoin::WalletLegacyTransfer& tr) {
  UpscaleCoin::WalletTransfer mtr;
...
void WalletSerializer::loadWallet(Common::IInputStream& source, const std::string& password, uint32_t version) {
  UpscaleCoin::CryptoContext cryptoContext;
...
void WalletSerializer::loadWalletV1(Common::IInputStream& source, const std::string& password) {
  ...
  UpscaleCoin::BinaryInputStreamSerializer encrypted(source);
  ...
  UpscaleCoin::BinaryInputStreamSerializer serializer(decryptedStream);
...
void WalletSerializer::loadWalletV1Keys(UpscaleCoin::BinaryInputStreamSerializer& serializer) {
  UpscaleCoin::KeysStorage keys;
...
void WalletSerializer::loadWalletV1Details(UpscaleCoin::BinaryInputStreamSerializer& serializer) {
...
void WalletSerializer::loadWallets(Common::IInputStream& source, CryptoContext& cryptoContext) {
  ...
  for (uint64_t i = 0; i < count; ++i) {
    ...
    wallet.container = reinterpret_cast<UpscaleCoin::ITransfersContainer*>(i); //dirty hack. container field must be unique
